...
for(i in names(chr)) {
  ...
  bam.param <- ScanBamParam(which = GRanges(i, IRanges(1, chr[i])),
                            ...
                            flag = scanBamFlag(isUnmappedQuery = FALSE, isPaired = FALSE, isFirstMateRead = TRUE, isMinusStrand = FALSE))
  open(bamFile)
  ...
  bam.param <- ScanBamParam(which = GRanges(i, IRanges(1, chr[i])),
                            what = c("qname", "flag", "mapq", "rname", "pos", "isize", "cigar", "qual"),
                            flag = scanBamFlag(isUnmappedQuery = FALSE, isPaired = FALSE, isFirstMateRead = TRUE, isMinusStrand = TRUE))
  open(bamFile)
